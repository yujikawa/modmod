...
### Requirement: Partition and Cluster Declaration
The `implementation` block SHALL support `partition_by` and `cluster_by` fields for warehouse-level physical optimization hints.
...
- **THEN** an AI agent SHALL apply clustering on `customer_id` and `region_id`
